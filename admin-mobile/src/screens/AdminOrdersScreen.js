      ...
      <ScrollView>
        ...
        {orders.map(order => (
          <View key={order.id} style={styles.card}>
            ...
            <Text style={styles.payment}>Payment: {String(order.paymentMethod || 'cash').toUpperCase()} · {order.paymentStatus || 'unpaid'}</Text>
            ...
            <View style={styles.bottomRow}>
              ...
              <TouchableOpacity disabled={['completed','cancelled'].includes(order.status)} style={[styles.btn, ['completed','cancelled'].includes(order.status) && styles.btnDisabled]} onPress={() => handleAdvance(order)}>
...
const styles = StyleSheet.create({
  ...
  payment: { fontSize: 12, color: COLORS.mocha, marginBottom: 6, fontWeight: '700' },
